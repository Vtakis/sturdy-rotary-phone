...
N=12,bucketPosNum=30

| Threads       | Time  | Max Memory(RAM) |   Heap   |
|:-------------:|:-----:|:---------------:|:--------:|
|        1      | 0.84s |          400 Kb | 13.03 Gb |
|        2      | 0.75s |          272 Kb | 13.03 Gb |
|        3      | 0.75s |          313 Kb | 13.03 Gb |
|        4      | 0.80s |          327 Kb | 13.03 Gb |
|        5      | 1.30s |          224 Kb | 13.03 Gb |

Σχόλιο:
> Δοκιμάσαμε για δίαφορα N και bucketPosNum και καταλήξαμε ότι για N=12,bucketPosNum=30 παίρνουμε σχετικά καλούς χρόνους γιαυτό οι παραπάνω μετρήσεις περιέχουν RAM και Heap. Το Heap το πείραμε τρέχοντας το πρόγραμμα με valgrind. Σαν Max Memory(RAM) εννοούμε την μέγιστη κατανάλωση μνήμης RAM που χρειάστηκε κατά την διάρκεια εκτέλεσης του προγράμματος.Το Heap είναι αρκετά μεγάλο γιατί χρησιμοποιούμε αρκετά μεγάλο Ν. Για μικρές τιμές του Ν το Heap μικραίνει αρκετά για παράδειγμα για Ν=5 Heap=1Gb.Χάνουμε σε μνήμη ,κερδίζουμε σε χρόνο!




Για τα αρχεία “public.*”
